...
func (s *PgCtldServerCmd) runServer(cmd *cobra.Command, args []string) error {
	...
	s.senv.OnRun(func() {
		...
		if s.grpcServer.CheckServiceMap(constants.ServicePgctld, s.senv) {
			...
			if err := pb.RegisterPgCtldHandlerServer(context.Background(), gwmux, pgctldService); err != nil { //nolint:gocritic // gRPC-gateway registration requires a background context at server startup
